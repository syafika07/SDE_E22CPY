...
export class TngPagec implements OnInit {
  ...
  progress = 0;
  private downloadProgressInterval: any = null;
...
applyFilter() {
  // 🔥 Clear interval dari downloadReport() jika masih aktif
  ...
  this.loading = true;
  this.progress = 0;  this.selectedPaymentModes = [];
  this.selectedTrx = [];
  this.selectedPenalties = [];
  this.selectedRemarks = [];
  this.currentPage = 1;
    ...
    next: (event) => {
      if (event.type === 0) { // HttpEventType.Sent
        ...
        if (total != null && total > 0) {
          // Hadkan nisbah antara 0 hingga 1
          const ratio = Math.min(1, loaded / total);
          this.progress = 10 + Math.floor(ratio * 60); // → maksimum 70
        } else {
          // Tiada total (Content-Length tidak dihantar oleh server)
          // Naikkan progress secara selamat sehingga 70%
          if (this.progress < 70) {
            this.progress = Math.min(70, this.progress + 2);
          }
        }
      }
