...
export class TngPage implements OnInit {
  ...
  // ✅ Apply Filter — hanya untuk fetch data baru apabila tarikh berubah
  applyFilter() {
    this.loading = true;
    this.progress = 0;
...
filterData() {
  ...
  onDateChange() {
    ...
    // ❗ TIDAK panggil applyFilter() di sini — tunggu user tekan button
  }
  ...
  toggleTrxSort() {
    ...
    this.filteredSubTable.sort((a, b) => {
      const aVal = a.TrxNo ?? '';
      const bVal = b.TrxNo ?? '';
  ...
  toggleDateSort() {
    ...
    this.filteredSubTable.sort((a, b) => {
      const aVal = new Date(a.TransactionDateTime).getTime();
      const bVal = new Date(b.TransactionDateTime).getTime();
