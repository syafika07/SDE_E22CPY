...
async updateFullUser(
  userId: string,
  updates: { email?: string; name?: string; role?: string }
) {
  ...
  if (updates.name) authUpdate.data = { display_name: updates.name };
  if (updates.email?.trim()) authUpdate.email = updates.email.trim();
...
async updateTransactionInDB(row: any) {
    ...
    try {
      ...
      this.logger.error('updateTransactionInDB error:', err);
      throw err;
    }
  }
  ...
  async getAnnouncements(): Promise<Announcement[]> {
    const { data, error } = await supabase
      .from('announcements') // tanpa generics untuk elak 'never'
      .select('*')
      .order('datetime', { ascending: true });

    if (error) throw error;

    // fallback null ke array kosong
    return (data || []) as Announcement[];
  }

  // ==========================
  // ADD ANNOUNCEMENT (single / multiple)
  // ==========================
  async addAnnouncement(announcement: Announcement | Announcement[]) {
    const list = Array.isArray(announcement) ? announcement : [announcement];
...
async updateAnnouncement(id: string, data: Partial<Announcement>) {
  console.log('[DEBUG] Updating announcement:', id, data);

  const { data: updated, error } = await supabase
    .from('announcements')
    .update(data)
    .eq('id', id)
    .select('*'); // pastikan data returned
  ...
  console.log('[DEBUG] Supabase update result:', updated);
...
async createAnnouncement(data: Partial<Announcement>) {
  ...
  async deleteAnnouncement(id: string) {
    ...
    return (deleted || []) as Announcement[];
  }
  // 🔐 Hantar email reset password
    async resetPasswordForEmail(email: string) {
      return await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/${ROUTES.RESET_PASS}`
      });
    }

// 🔐 Update password selepas klik email
