...
async def upload_csv(files: list[UploadFile] = File(...), preview: bool = Query(False)):
    try:
        ...
        # 🔴 TIADA PEMBANDINGAN DENGAN DATA SEDIA ADA DI SINI — terus insert semua
        ...
        # 🔴 Assign ID baru berdasarkan MAX(id) — tetap perlukan untuk `id` column
        ...
        # 🔴 INSERT SEMUA — termasuk duplikat (akan dibersihkan selepas)
        df.to_sql("sde22", engine, schema="public", if_exists="append", index=False)

        # ✅ DEDUPLICATION: Hanya selepas insert
        with engine.begin() as conn:
            conn.execute(text("""
                WITH duplicates AS (
                    SELECT id,
        ...
        return {
            "status": "success",
            "rows_inserted_before_dedup": len(df),
            "message": "Data berjaya dimasukkan. Duplikat dihapuskan dalam database.",
            "download": None
        }
...
async def upload_pdf(files: list[UploadFile] = File(...), preview: bool = Query(False)):
    ...
    return {
        "status": "success",
        "rows_inserted_before_dedup": len(df),
        "message": "Data PDF berjaya dimasukkan. Duplikat dihapuskan dalam database.",
        "download": None
    }
